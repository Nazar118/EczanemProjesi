...
import { 
  ...
  IonSearchbar, IonIcon, IonSpinner, useIonRouter 
} from '@ionic/react';
import { notificationsOutline, medkitOutline, leafOutline, heartOutline, heart, cartOutline, happyOutline, nutritionOutline, waterOutline, gridOutline, chevronForwardOutline } from 'ionicons/icons';
    ...
    const fetchData = async () => {
      try {
        ...
        if (categoriesData && Array.isArray(categoriesData)) {
            // 🔥 'any' HATASI DA BURADA DÜZELTİLDİ
            const vitrinKategorileri = categoriesData.filter((cat: Category) => cat.type === 'Özel Etiket');
  ...
  return (
    <IonPage>
      <IonHeader className="ion-no-border">
        ...
        <IonToolbar style={{ '--background': '#ffffff', paddingBottom: '10px' }}>
          <IonSearchbar 
            ...
            placeholder="İlaç, vitamin veya bakım ürünü ara..." 
            style={{ padding: '0 15px', '--border-radius': '12px', '--box-shadow': 'none', '--background': '#f4f5f8' }} 
          />
        </IonToolbar>
      </IonHeader>
      ...
      <IonContent className="ion-padding" style={{ '--background': '#f8f9fa' }}>
        ...
        {searchText.trim() !== '' ? (
          <div>
            <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#111', marginBottom: '15px' }}>Arama Sonuçları</h2>
            {filteredProducts.length === 0 ? (
              <p style={{ color: '#888', fontStyle: 'italic' }}>"{searchText}" için sonuç bulunamadı.</p>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                {filteredProducts.map((product) => (
                  <div key={`search-${product.id}`} style={{ backgroundColor: 'white', borderRadius: '18px', padding: '12px', boxShadow: '0 2px 10px rgba(0,0,0,0.03)', display: 'flex', flexDirection: 'column' }}>
                    <IonIcon 
          ...
          <>
                  ...
                  categories.map((category) => {
                    ...
                    return (
                      <div 
                        ...
                        // 🔥 İŞTE SİHİR BURADA! TIKLANDIĞINDA DOĞRUDAN O KATEGORİNİN ÜRÜNLERİNE GİDECEK
                        onClick={() => router.push(`/app/category-products/${category.id}`, 'forward')} 
                      >
                        <div style={{ 
            ...
            <div style={{ marginTop: '15px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                ...
                <div 
                  onClick={() => router.push('/app/showcase/bestsellers')} 
                  style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}
                >
                  <span style={{ fontSize: '13px', color: 'var(--ion-color-primary)', fontWeight: '600' }}>Tümünü Gör</span>
                  <IonIcon icon={chevronForwardOutline} style={{ color: 'var(--ion-color-primary)', fontSize: '14px', marginLeft: '2px' }} />
                </div>
              </div>
              ...
              <div style={{ display: 'flex', overflowX: 'auto', gap: '12px', paddingBottom: '8px', msOverflowStyle: 'none', scrollbarWidth: 'none', scrollSnapType: 'x mandatory' }}>
                {loading ? (
                  ...
                  products.filter(p => p.isBestSeller === true).map((product) => (
                    <div key={product.id} style={{ minWidth: '145px', maxWidth: '145px', backgroundColor: 'white', borderRadius: '18px', padding: '12px', boxShadow: '0 2px 10px rgba(0,0,0,0.03)', scrollSnapAlign: 'start', display: 'flex', flexDirection: 'column', position: 'relative' }}>
                      
                      <IonIcon 
                        icon={isFavorite(product.id) ? heart : heartOutline} 
                        style={{ position: 'absolute', top: '12px', right: '12px', fontSize: '22px', color: isFavorite(product.id) ? '#ef4444' : '#cbd5e1', zIndex: 2, cursor: 'pointer', transition: 'all 0.3s' }} 
                        onClick={() => toggleFavorite(product.id)}
                      />
            ...
            <div style={{ marginTop: '20px', marginBottom: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                ...
                <div 
                  onClick={() => router.push('/app/showcase/recommended')} 
                  style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}
                >
                ...
                {products.filter(p => p.isRecommended === true).length === 0 && !loading ? (
                  ...
                  products.filter(p => p.isRecommended === true).map((product) => (
                    <div key={`seckin-${product.id}`} style={{ 
                      ...
                      <IonIcon 
                        icon={isFavorite(product.id) ? heart : heartOutline} 
                        style={{ position: 'absolute', top: '8px', right: '10px', fontSize: '20px', color: isFavorite(product.id) ? '#ef4444' : '#cbd5e1', cursor: 'pointer' }} 
                        onClick={() => toggleFavorite(product.id)}
                      />
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', overflow: 'hidden', paddingRight: '20px' }}>
